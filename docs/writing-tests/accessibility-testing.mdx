...
Storybook provides an Accessibility (a11y) addon to help ensure the accessibility of your components. The addon supports two accessibility testing engines:

- **axe-core** (default): Built on Deque's [axe-core library](https://github.com/dequelabs/axe-core), which automatically catches [up to 57% of WCAG issues](https://www.deque.com/blog/automated-testing-study-identifies-57-percent-of-digital-accessibility-issues/)
- **IBM Equal Access**: IBM's comprehensive accessibility checker with support for multiple accessibility policies

You can choose which engine to use based on your project's requirements.
...
## Choose an accessibility engine

The addon supports two accessibility testing engines. You can configure which engine to use in your Storybook configuration:

### axe-core (default)

The default engine, based on Deque's industry-standard axe-core library. It provides comprehensive WCAG 2.0 and 2.1 testing with customizable rulesets.

<CodeSnippets path="addon-a11y-config-in-preview.md" />

### IBM Equal Access

IBM's accessibility checker with support for multiple accessibility policies including IBM Accessibility guidelines and WCAG standards.
...
export default {
  parameters: {
    a11y: {
      engine: 'equal-access',
      config: {
        // Specify which accessibility policies to test against
        policies: ['IBM_Accessibility'], // or 'WCAG_2_1', 'WCAG_2_0'
        rules: {
          'aria_content_in_landmark': { enabled: false }
        }
      }
    }
  }
...
### Understanding severity badges

The addon displays severity badges to help you prioritize accessibility issues. The badge colors and meanings differ between engines:
...
In the **Incomplete** tab, badges indicate potential severity:
- **axe-core**: Shows the rule's potential impact if it were to fail (Critical, Serious, Moderate, Minor)
- **IBM Equal Access**: Shows Potential Violation, Potential Recommendation, or Needs Review

## Configure

The addon provides configuration options for both engines. Some options are engine-specific:

### Common configuration options

| Property                  | Default     | Description                                                                                                                                                                         |
| ------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `parameters.a11y.engine`  | `'axe-core'` | Choose the accessibility engine: `'axe-core'` or `'equal-access'`                                                                                                                  |
| `parameters.a11y.context` | `'body'`    | Defines which elements to run checks against. Works with both engines.                                                                                                              |
| `parameters.a11y.config`  | (see below) | Engine-specific configuration. Most commonly used to [configure individual rules](#individual-rules).                                                                               |
| `parameters.a11y.options` | `{}`        | [Options](https://github.com/dequelabs/axe-core/blob/develop/doc/API.md#options-parameter) passed to `axe.run` (axe-core only).                                                    |
...
<details>
<summary>Default `parameters.a11y.config` for axe-core</summary>
...
For IBM Equal Access, you can configure:
- `policies`: Array of accessibility policies to test against (e.g., `['IBM_Accessibility', 'WCAG_2_1']`)
- `reportLevels`: Filter results by severity (e.g., `['violation', 'potentialviolation', 'recommendation']`)
- `rules`: Enable/disable individual rules (same format as axe-core)

<details>
<summary>Default `parameters.a11y.config` for IBM Equal Access</summary>
...
The axe-core engine runs a set of rules based on WCAG 2.0 and 2.1 guidelines, as well as best practices:
...
You can find a breakdown of these rulesets, as well as other available rulesets in [axe-core's documentation](https://github.com/dequelabs/axe-core/blob/develop/doc/rule-descriptions.md#wcag-2x-level-aaa-rules).
...
For IBM Equal Access, use the `policies` parameter to select which accessibility guidelines to test against:
...
export default {
  parameters: {
    a11y: {
      engine: 'equal-access',
      config: {
        // Test against WCAG 2.1 Level A & AA
        policies: ['WCAG_2_1']
      }
    }
  }
...
You can specify multiple policies to test against multiple standards simultaneously.

### Individual rules

You can enable, disable, or configure individual rules for both engines. This is done in the `config` property of the `parameters.a11y` object.
...
**Note:** IBM Equal Access does not support per-rule options like axe-core does. You can only enable or disable rules. For more granular control, use the `policies` parameter to select stricter or more lenient accessibility standards.
...
### Which accessibility engine should I use?

**Use axe-core if:**
- You need the industry-standard accessibility testing tool
- You want extensive customization options (custom rules, checks, etc.)
- You're already familiar with axe-core
- You need per-rule configuration options

**Use IBM Equal Access if:**
- You need to comply with IBM Accessibility guidelines
- You want policy-based testing (IBM, WCAG 2.0, WCAG 2.1)
- You prefer IBM's comprehensive accessibility checker
- You need detailed manual review guidance

Both engines are fully supported and can be used interchangeably. You can switch between them at any time by changing the `engine` parameter.

### Can I use both engines in the same project?

Yes! You can configure different engines for different stories or components. For example:

```ts
// Use axe-core globally in .storybook/preview.ts
export default {
  parameters: {
    a11y: {
      engine: 'axe-core'
